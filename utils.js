...
// Goal calculation utilities for consistency across components
export const GoalUtils = {
    /**
     * Calculate goals for a player from match data (excluding own goals)
     * This is the consistent method to use across all components
     */
    countPlayerGoalsFromMatches(playerName, playerTeam, matches) {
        let totalGoals = 0;
        ...
        matches.forEach(match => {
            ...
            if (playerTeam === 'AEK' && match.goalslista) {
                ...
                goals.forEach(goal => {
                    ...
                    // Exclude own goals (starting with "Eigentore_")
                    if (goalPlayer === playerName && !goalPlayer.startsWith('Eigentore_')) {
    ...
    /**
     * Calculate enhanced player stats with goals from matches
     */
    calculatePlayersWithMatchGoals(players, matches) {
        ...
        return players.map(player => ({
            ...player,
            // Override goals with calculated goals from matches
            goals: this.countPlayerGoalsFromMatches(player.name, player.team, matches)
        }));
    },

    /**
     * Get top scorers sorted by goals from matches
     */
    getTopScorers(players, matches) {
        const playersWithMatchGoals = this.calculatePlayersWithMatchGoals(players, matches);
        return playersWithMatchGoals
